...
const market_equilibrioSocketPath = ws_scheme + '://' + window.location.host + "/market_equilibrio/" + group_pk + '/' + player_pk;
const market_equilibrioSocket = new WebSocket(market_equilibrioSocketPath);
...
market_equilibrioSocket.onmessage = function (e) {
...
market_equilibrioSocket.onclose = function (e) {
...
const changeprice = () => {
    ...
    if (validar_rango( new_value)) {
        document.getElementById("precio").value = ''
        market_equilibrioSocket.send(JSON.stringify({
            'bid_up': true,
