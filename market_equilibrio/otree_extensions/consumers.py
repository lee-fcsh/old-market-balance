# market_equilibrio/consumers.py
from channels.generic.websocket import WebsocketConsumer
import json
from asgiref.sync import async_to_sync
from termcolor import colored
from market_equilibrio.models import Player, Group, Constants, Subsession
import time


class Market_equilibrioConsumer(WebsocketConsumer):
    def connect(self):
        self.group_pk = int(self.scope['url_route']['kwargs']['group_pk'])
        self.player_pk = int(self.scope['url_route']['kwargs']['player_pk'])
        self.room_group_name = 'market_equilibrio_%s' % self.group_pk
